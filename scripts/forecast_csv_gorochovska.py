...
if __name__ == "__main__":
    ...
    generate_forecast(
        init_time_freq=6,
        start_datetime="2024-05-06 00:00:00",
        end_datetime="2024-05-08 00:00:00",
        site_name="testGorochivska",
        latitude=46.98,
        longitude=-32.10,
        capacity_kwp=12.50
    )
...
class TestGenerateForecast(unittest.TestCase):
    def setUp(self):
        self.site_name = "TestCaseGorochivska"
        self.latitude = 46.98
        self.longitude = -32.10
        self.capacity_kwp = 12000.500
        self.start_datetime = "2024-05-06 00:00:00"
        self.end_datetime = "2024-05-08 00:00:00"
    ...
    def test_generate_forecast(self, mock_run_forecast):
        mock_df = pd.DataFrame({
            'datetime': [datetime(2024, 5, 6, 0, 0) + timedelta(hours=6 * i) for i in range(4)],
            'power_kw': [0.1, 0.5, 0.8, 0.6],
            'forecast_init_time': [datetime(2024, 5, 6, 0, 0)] * 4
        })
